...
class _BroadcastMessageMethods(_BaseMessageMethods):
    ...
    def security_group_create(self, message, group):
        ...
        LOG.error(_("Received message to create group %(group_dict)s") % locals())
    ...
    def security_group_rule_create(self, message, rule):
        ...
        LOG.error(_("Received message to create rule %(rule_dict)s"), locals())
        ...
        if not existing_rule:
            LOG.error(_("Adding missing security group rule %s" % ((dict(rule.iteritems())))))
    ...
    def _association_exists(self, ctxt, instance_uuid, group):
        group = self.db.security_group_get_by_name(
            ctxt,
            group['project_id'],
            group['name']
    ...
    def instance_add_security_group(self, message, instance_uuid, group):
        ...
        try:
            exists = self._association_exists(ctxt, instance_uuid, group)
        except exception.SecurityGroupNotFound:
...
class MessageRunner(object):
    ...
    def _security_group_rule_create_or_destroy(self, ctxt, group, rule,
        ...
        if linked_id:
            ...
            linked_group = self.db.security_group_get(rd_ctxt, linked_id)
